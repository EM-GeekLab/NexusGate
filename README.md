...
![Banner](./assets/img/banner-cn.svg)
...
NexusGate 是一款开源、轻量的 **AI Agent 和 LLM 应用**监控与管理工具，可以一键构建您企业“智能基础设施”的“管理面板”与“任务管理器”。
...
- **多提供商支持**：支持 OpenAI、Anthropic、Azure OpenAI、Ollama 等多种 LLM 提供商，以及新型 OpenAI Responses API 格式。
- **函数调用支持**：完整支持工具定义、函数调用和工具结果的记录与展示，方便调试 Agent 应用。
- **智能负载均衡**：支持加权随机负载均衡，可在多个提供商之间分配流量。
- **自动故障转移**：当某个提供商失败时，自动切换到备用提供商，支持指数退避重试策略。
- **生产监控**：记录所有生产交互以进行监控、分析、调试、优化。
- **企业级管理**：一键管理应用程序或业务系统，提供 Agent 内容的计量和审计。
- **Kubernetes 原生**：提供 K8s Operator，支持在 Kubernetes 环境中自动管理 API 密钥。
...
2. **配置环境变量（重要）**
   ...
   # 服务配置
   # ======================
   # Web 服务对外端口（可选，不设置时默认 3000，建议设置为 8080）
   WEB_PORT=8080
   EOF
   ```
   ...
   **重要参数说明：**
   ...
   | `WEB_PORT`           | ❌    | Web 服务端口（不设置时默认 3000） | `8080`                  |
...
```
https://your-nexusgate-server/api/openapi.json
```
...
- [x] 🌐 国际化：完成 i18n 支持，提供官方中文支持。
- [ ] 📊 增强分析：扩展我们的监控指标，包括成功率、请求量、Token 使用统计、请求完成率、Agent 使用量排行、模型使用量排行、错误分析、全链路延迟、推理延迟和吞吐量等测量。
- [x] 🔄 Prometheus 集成：通过与外部 Prometheus 实例集成，监控服务器硬件、推理框架和其他信息源，创建全面的概览仪表板。
- [x] 🚦 流量控制：为每个 API 密钥实现精细的流量管理，包括特定模型的配额和优先级，使管理员能够精确控制资源分配。
- [x] ⚖️ 智能负载均衡：支持加权随机负载均衡，可在多个提供商之间按权重分配流量。
- [x] 🔁 自动故障转移：当某个提供商失败时，自动切换到备用提供商，支持指数退避重试策略。
- [x] 🛠️ 函数调用支持：完整支持 OpenAI 格式的工具定义、函数调用（tool_calls）和工具结果的记录与展示。
- [x] ☸️ Kubernetes Operator：提供 K8s Operator，支持在 Kubernetes 环境中自动管理 API 密钥。
...
- [ ] 🌐 网关层工具集成：通过在网关层实现功能并通过标准 API 接口暴露，为没有内置工具（如网络搜索）的模型添加功能。
